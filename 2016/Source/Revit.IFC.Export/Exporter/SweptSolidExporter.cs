         ...
         try
         {
            ...
            if (sweptAnalyzer.PathCurve is Line)
            {
              ...
              line.Direction, UnitUtil.ScaleLength(line.Length), false);
               ...
               {
                  sweptSolidExporter.FootprintHandle = GeometryUtil.CreateIFCCurveFromCurveLoop(exporterIFC, faceBoundaries[0], plane, line.Direction);
               }
            }
            else
            {
               ...
               if (ExporterCacheManager.ExportOptionsCache.ExportAs4)
               {
                  ...
                  else
                  {
                     ...
                     if ((addInfo & GenerateAdditionalInfo.GenerateFootprint) != 0)
                     {
                        Plane planeFace = new Plane(sweptAnalyzer.ReferencePlaneNormal, sweptAnalyzer.ProfileFace.Origin);
                        sweptSolidExporter.FootprintHandle = GeometryUtil.CreateIFCCurveFromCurveLoop(exporterIFC, faceBoundaries[0], planeFace, sweptAnalyzer.ReferencePlaneNormal);
                     }
                  }
               }
               else
