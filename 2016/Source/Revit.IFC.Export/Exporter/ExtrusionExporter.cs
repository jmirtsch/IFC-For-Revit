...
namespace Revit.IFC.Export.Exporter
{
   ...
   class ExtrusionExporter
   {
      ...
      private static HandleAndAnalyzer CreateExtrusionWithClippingBase(ExporterIFC exporterIFC, Element element,
            ElementId catId, IList<Solid> solids, Plane plane, XYZ projDir, IFCRange range, out bool completelyClipped, out HashSet<ElementId> materialIds, GenerateAdditionalInfo addInfo = GenerateAdditionalInfo.None)
      {
         IFCFile file = exporterIFC.GetFile();
         ...
         using (IFCTransaction tr = new IFCTransaction(file))
         {
            ...
            foreach (Solid solid in solids)
            {
              ...
              out completelyClipped, out hasClippingResult, out hasBooleanResult, out materialId, addInfo: addInfo);
               ...
               if (!IFCAnyHandleUtil.IsNullOrHasNoValue(extrusionBodyItemHnd))
               {
                  ...
                  if (ExporterCacheManager.ExportOptionsCache.ExportAs4)
                  {
                     if ((addInfo & GenerateAdditionalInfo.GenerateFootprint) != 0)
                     {
                        ...
                        Plane footprintPlane = new Plane(XYZ.BasisZ, XYZ.Zero);

                        IFCAnyHandle footprintGeomRepItem = GeometryUtil.CreateIFCCurveFromCurveLoop(exporterIFC, extrusionBoundaryLoops[0], footprintPlane, extrusionBasePlane.Normal);
          ...
          GenerateAdditionalInfo addInfo = GenerateAdditionalInfo.None)
      {
         ...
         HandleAndAnalyzer handleAndAnalyzer = CreateExtrusionWithClippingBase(exporterIFC, element, catId,
             solids, plane, projDir, range, out completelyClipped, out materialIds, addInfo: addInfo);
         if ((addInfo & GenerateAdditionalInfo.GenerateFootprint) != 0)
         {
