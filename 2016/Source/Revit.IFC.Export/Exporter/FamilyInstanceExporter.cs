               ...
               if (!exportParts)
               {
                     ...
                     if (exportType == IFCExportType.IfcColumnType || exportType == IFCExportType.IfcMemberType || exportType == IFCExportType.IfcBeamType)
                     {
                        ...
                        if (locCurve != null)
                        {
                           ...
                           if (curve is Line)
                           {
                              ...
                              basePlane = new Plane(planeY, planeZ, XYZ.Zero);
                           }
                           else if (curve is Arc)
                           {
                              ...
                              basePlane = new Plane(extrudeDirection, orig);
                           }
                           else
                           {
                        ...
                        {
                           Plane clipPlane = new Plane(basePlane.XVec, basePlane.YVec, orig);
                           ...
                           IFCAnyHandle bodyRepresentation = ExtrusionExporter.CreateExtrusionWithClipping(exporterIFC, exportGeometryElement,
                               categoryId, solids, clipPlane, extrudeDirection, null, out completelyClipped, out materialIds,
                  ...
                  if (needToCreate2d)
                  {
                     ...
                     HashSet<IFCAnyHandle> curveSet = new HashSet<IFCAnyHandle>();
                     {
                        ...
                        Plane curvePlane = new Plane(planeTrf.BasisX, planeTrf.BasisY, planeTrf.Origin);

                        IFCGeometryInfo IFCGeometryInfo = IFCGeometryInfo.CreateCurveGeometryInfo(exporterIFC, curvePlane, projDir, true);
