...
namespace BIM.IFC.Export.UI
{
   ...
   public partial class IFCExport : ChildWindow
   {
      ...
      /// <summary>
      /// Force the current IFC window to be "on top" of the main Revit window.   This allows the IFC UI to show on top when you switch to Revit from another application.
      /// </summary>
      /// <param name="currentWindow">The UI window to make the current "main" window.</param>
      static public void MakeCurrentRevitWindow(ChildWindow currentWindow)
      {
         WindowInteropHelper windowHelper = new WindowInteropHelper(currentWindow);
         if (windowHelper != null)
         {
            Process currentProcess = Process.GetCurrentProcess();
            if (currentProcess != null)
            {
               windowHelper.Owner = currentProcess.MainWindowHandle;
            }
         }
      }
      ...
      private void buttonNext_Click(object sender, RoutedEventArgs args)
      {
         ...
         else
         {
            ...
            {
               MessageBoxResult msgBoxResult = MessageBox.Show(String.Format(Properties.Resources.FileExists, textBoxSetupFileName.Text), Properties.Resources.IFCExport, MessageBoxButton.YesNo);
