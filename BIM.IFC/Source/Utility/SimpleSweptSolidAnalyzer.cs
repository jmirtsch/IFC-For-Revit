...
namespace BIM.IFC.Utility
{
    ...
    class SimpleSweptSolidAnalyzer
    {
        ...
        static Autodesk.Revit.Creation.Application m_appCreation = null;
        ...
        private static List<Face> FindAlignedFaces(ICollection<Face> profileFaces)
        {
            ...
            foreach (Face face in profileFaces)
            {
                ...
                for (int i = 0; i < edgeLoopCount; i++)
                {
                    ...
                    {
                        Face alignedFace = edge.get_Face(0);
                        if (alignedFace == face)
                            alignedFace = edge.get_Face(1);
        ...
        private static List<Edge> FindCandidatePathEdge(Face face, Edge edge0, Edge edge1)
        {
            ...
            XYZ[,] endPoints = new XYZ[2, 2] { { curve0.get_EndPoint(0), curve1.get_EndPoint(1) }, { curve0.get_EndPoint(1), curve1.get_EndPoint(0) } };
