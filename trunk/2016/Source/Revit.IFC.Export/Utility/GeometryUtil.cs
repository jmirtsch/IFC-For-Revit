...
namespace Revit.IFC.Export.Utility
{
    ...
    public class GeometryUtil
    {
        ...
        public static Dictionary<EdgeArray, IList<EdgeArray>> SortEdgeLoop(EdgeArrayArray edgeArrays, Face face) 
        {
            ...
            Dictionary<IList<UV>, IList<IList<UV>>> loopCollections = new Dictionary<IList<UV>,IList<IList<UV>>>();
            ...
            foreach (EdgeArray edgeArray in edgeArrays) 
            {
                ...
                foreach (KeyValuePair<IList<UV>, IList<IList<UV>>> entry in loopCollections)
                {
                    ...
                    if (PointInsidePolygon(tessellatedLoop[0], entry.Key))
                    {
                        ...
                        IList<UV> outerLoop = new List<UV>();
                        IList<IList<UV>> innerLoops = new List<IList<UV>>();
                        if (IsInsideAnotherLoop(tessellatedLoop, entry.Value, out outerLoop))
                        {
                            // if tessellateLoop is inside another loop, then it will become the outer loop
                            loopCollections.Add(tessellatedLoop, new List<IList<UV>>());
                        }
                        else if (IsOutsideAnotherLoops(tessellatedLoop, entry.Value, out innerLoops))
                        {
                            ...
                            foreach (IList<UV> innerLoop in innerLoops)
                            {
                                ...
                                loopCollections.Add(innerLoop, new List<IList<UV>>());
                            }
                        }
                        else 
                        {
        ...
        private static bool IsInsideAnotherLoop(IList<UV> loopToCheck, IList<IList<UV>> listOfLoops, out IList<UV> resultedLoop) 
        {
            resultedLoop = new List<UV>();
        ...
        private static bool IsOutsideAnotherLoops(IList<UV> loopToCheck, IList<IList<UV>> listOfLoops, out IList<IList<UV>> resultedList) 
        {
        ...
        public static bool PointInsidePolygon(UV pnt, IList<UV> polyNodes) 
        {
            if (pnt == null || polyNodes == null || polyNodes.Count == 0)
                return false;

            int nNodes = polyNodes.Count;
            // keep code of two pntInsidePolygon() in synch!
