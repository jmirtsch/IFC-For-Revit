...
namespace Revit.IFC.Export.Exporter
{
    ...
    class ExporterApplication : IExternalDBApplication
    {
        ...
        {
            bool isRegistered = ExporterIFCRegistry.IsIFCExporterRegistered();
            if (!isRegistered)
            {
                ExporterIFCRegistry.RegisterIFCExporter(new Exporter());
            }
        }
    }
    ...
    public class Exporter : IExporterIFC
    {
        ...
        protected virtual IFCFileModelOptions CreateIFCFileModelOptions(ExporterIFC exporterIFC)
        {
            ...
            {
                // This is IFC4 Release Candidate 4, to be replaced by the final version when it is available.
                modelOptions.SchemaFile = Path.Combine(ExporterUtil.RevitProgramPath, "EDM\\IFC4RC4.exp");
                modelOptions.SchemaName = "IFC4RC4";
            }
            else
            {
        ...
        private void EndExport(ExporterIFC exporterIFC, Document document)
        {
            ...
            using (IFCTransaction transaction = new IFCTransaction(file))
            {
                ...
                using (ProductWrapper productWrapper = ProductWrapper.Create(exporterIFC, true))
                {
                    ...
                    if (projectInfo != null)
                    {
                        ExportElementProperties(exporterIFC, projectInfo, productWrapper);
                        PropertyUtil.CreateInternalRevitPropertySets(exporterIFC, projectInfo, productWrapper);
                    }
                }
                ...
                foreach (SpaceInfo spaceInfo in ExporterCacheManager.SpaceInfoCache.SpaceInfos.Values)
                {
                    ...
                    {
                        IFCInstanceExporter.CreateRelContainedInSpatialStructure(file, GUIDUtil.CreateGUID(), exporterIFC.GetOwnerHistoryHandle(),
        ...
        private HashSet<IFCAnyHandle> CreateContextInformation(ExporterIFC exporterIFC, Document doc)
        {
            ...
            double scaledPrecision = UnitUtil.ScaleLength(precision);
            if (scaledPrecision > precision)
            {
                int exponent = ((int)(Math.Log10(scaledPrecision) - 0.01)) + 1;
        ...
        private IFCAnyHandle CreateDefaultUnits(ExporterIFC exporterIFC, Document doc)
        {
            ...
            double areaScaleFactor = 1.0;
            double volumeScaleFactor = 1.0;
            ...
            IFCAnyHandle lenSIBaseUnit = null;
            {
                ...
                lengthScaleFactor = UnitUtils.ConvertFromInternalUnits(1.0, lenUseDefault ? DisplayUnitType.DUT_DECIMAL_FEET : lenFormatOptions.DisplayUnits);
                ...
                {
                    double lengthSIScaleFactor = lengthScaleFactor * UnitUtils.ConvertFromInternalUnits(1.0, DisplayUnitType.DUT_METERS);
                ...
                ExporterCacheManager.UnitsCache.AddUnit(UnitType.UT_Length, lenSIUnit, lengthScaleFactor);
            }

            {
                ...
                areaScaleFactor = UnitUtils.ConvertFromInternalUnits(1.0, areaUseDefault ? DisplayUnitType.DUT_SQUARE_FEET : areaFormatOptions.DisplayUnits);
                ...
                ExporterCacheManager.UnitsCache.AddUnit(UnitType.UT_Area, areaSiUnit, areaScaleFactor);
            }

            {
                ...
                volumeScaleFactor = UnitUtils.ConvertFromInternalUnits(1.0, volumeUseDefault ? DisplayUnitType.DUT_CUBIC_FEET : volumeFormatOptions.DisplayUnits);
                ...
                ExporterCacheManager.UnitsCache.AddUnit(UnitType.UT_Volume, volumeSiUnit, volumeScaleFactor);
            }

            {
                ...
                ExporterCacheManager.UnitsCache.AddUnit(UnitType.UT_Angle, planeAngleUnit, 1.0 / angleScaleFactor);
            }
